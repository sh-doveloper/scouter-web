...
export default function MergeLineChart({ setMergeGrowthRate = () => {} }) {
  // console.log('setMergeGrowthRate:', typeof setMergeGrowthRate == 'function');
  ...
  const loadChartData = async () => {
    setLoading(true);
    try {
      const response = await fetchMergeChartData();

      // X축 날짜를 'MM.DD' 형식으로 변환
      const categories = response.dailyMergeCounts.map((item) => item.mergeDate.slice(5).replace(/-/g, '.'));

      // 데이터 변환
      const seriesData = response.dailyMergeCounts.map((item, index) => ({
        x: categories[index],
        y: item.mergeCount,
      ...
      if (tickAmount > 5) tickAmount = 4; // 최대 10개 눈금
  ...
  return (
    <>
      <Box sx={{ pt: 1, pr: 2 }}>
        {loading ? <p>Loading...</p> : <ReactApexChart options={options} series={series} type="line" height={200} />}
      </Box>
    </>
  );
}
